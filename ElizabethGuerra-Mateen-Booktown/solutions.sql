...
booktown=# SELECT * FROM subjects ORDER BY subject ASC;
--  id |     subject      |     location
-- ----+------------------+------------------
...
booktown=# SELECT * FROM subjects ORDER BY location ASC;
--  id |     subject      |     location
-- ----+------------------+------------------
--   8 | History          | Academic Rd
--   3 | Classics         | Academic Rd
...
-- booktown=# SELECT b.title FROM books b INNER JOIN subjects s ON b.subject_id=s.id WHERE s.subject='Computers';
--         title
-- ----------------------
...
booktown=# SELECT b.title, a.first_name, a.last_name, s.subject FROM subjects s RIGHT JOIN books b ON s.id = b.subject_id LEFT JOIN authors a ON a.id = b.author_id;
...
booktown=# SELECT b.title, s.retail FROM books b INNER JOIN editions e ON b.id=e.book_id INNER JOIN stock s ON s.isbn = e.isbn ORDER BY s.retail DESC;
--             title            | retail
-- -----------------------------+--------
...
booktown=# SELECT b.title, s.isbn, p.name, s.retail FROM books b INNER JOIN editions e ON b.id=e.book_id INNER JOIN stock s ON s.isbn = e.isbn INNER JOIN publishers p ON p.id = e.publisher_id WHERE b.title = 'Dune';
...
booktown=# SELECT c.first_name, c.last_name, s.ship_date, b.title FROM shipments s INNER JOIN customers c ON s.customer_id = c.id INNER JOIN editions e ON s.isbn = e.isbn INNER JOIN books b ON b.id = e.book_id ORDER BY s.ship_date ASC;
...
booktown=# SELECT b.id, b.title, COUNT(e.edition) FROM books b LEFT JOIN editions e ON b.id=e.book_id GROUP BY b.id;
--   id   |            title            | count
-- -------+-----------------------------+-------
