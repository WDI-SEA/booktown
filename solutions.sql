...
SELECT * FROM subjects WHERE location LIKE 'Main St%' OR location LIKE '%Main St' ORDER BY subject;
...
-- 	* ship date
-- 	* book title
SELECT first_name, last_name, ship_date, title FROM shipments JOIN customers ON shipments.customer_id = customers.id JOIN editions ON editions.isbn = shipments.isbn JOIN books ON editions.book_id = books.id ORDER BY ship_date;
...
SELECT book_id, title, COUNT(edition) FROM editions JOIN books ON editions.book_id = books.id GROUP BY book_id;
